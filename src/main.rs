//! monitor-agent-freebsd: reports one FreeBSD host to a monitor hub over
//! WebSocket.
//!
//! The protocol half of this file is carried over from the official Linux
//! agent (https://github.com/monitor-probe/agent, MIT); only the parts that
//! touched Linux-specific startup diagnostics were removed. The collection
//! layer it drives lives in `collect.rs` and speaks to the FreeBSD kernel.
...
fn usage() -> ! {
    eprintln!(
        "monitor-agent-freebsd {}\n\n\
         Usage: monitor-agent-freebsd --server <url> --token <token> [options]\n\n\
         Options:\n  \
           ...
           --iface <list>       Count traffic on these interfaces alone, e.g.\n                       \
                                em0,vtnet0. `-name` removes an interface from\n                       \
                                what would be counted. Full names only. By\n                       \
                                default every interface is counted, which in\n                       \
...
async fn main() -> Result<()> {
    ...
    // such as a link not yet up, is counted once it appears.
...
/// retransmit.
///
/// FreeBSD arms its initial SYN timer at one second, as Linux does. A longer
/// wait turns a dropped SYN into a late success, reporting the retransmit
/// timer plus the round trip as latency.
///
...
///
/// Every failure advances to the next address, refusals included.
